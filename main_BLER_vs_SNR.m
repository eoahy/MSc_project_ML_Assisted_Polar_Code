% main_BLER_vs_SNR(code, A, E, L, min_sum, target_block_errors, target_BLER, EsN0_start, EsN0_delta, seed,Kfactor,Mlevel)
...
% M-PSK
 ...
 Mlevel = 4;  
 index=[0:Mlevel-1];
 modulation_symbols = exp(1i*2*pi*index/Mlevel);

% 16QAM
%modem_name = '16QAM';
%Mlevel = 16; 
%modulation_symbols = sqrt(1/10)*[-3+3*i, -1+3*i, +1+3*i, +3+3*i, -3+1*i, -1+1*i, +1+1*i, +3+1*i, -3-1*i, -1-1*i, +1-1*i, +3-1*i, -3-3*i, -1-3*i, +1-3*i, +3-3*i];


frame_bit_length = E;
bps = log2(Mlevel);   % bits per modulated symbol    
symbol_frame_length = frame_bit_length/bps; % number of symbols per frame
...
% main_BLER_vs_SNR('PUCCH', 84, 136, 8,1, 1000, 1e-5, -12 ,0.5, 0, 2)
...
for A_index = 1:length(A)
    ...
    title([code, ' SCL polar code, A = ',num2str(A(A_index)),', L = ',num2str(L),', minsum = ',num2str(min_sum),', errors = ',num2str(target_block_errors),', ',modem_name,' ', channel_name]);
    ylabel('BLER');
    xlabel('E_s/N_0 [dB]');
    ylim([target_BLER,1]);
    hold on
    ...
    for E_index = 1:length(E)
        ...
        filename = ['results/BLER_vs_SNR_',code,'_',num2str(A(A_index)),'_',num2str(E(E_index)),'_',num2str(L),'_',num2str(min_sum),'_',num2str(target_block_errors),'_',num2str(EsN0_start),'_',num2str(seed)];
        ...
        try
            ...
            while BLER > target_BLER
                ...
                while keep_going && block_error_counts(end) < target_block_errors
                    ...
                    % bit to integer conversion: two options 
                    symbols = bi2de(reshape(f,bps,symbol_frame_length)','right-msb');  
        
                    % modulation                    
                    Modem_symbols =  modulation_symbols(symbols+1);

                    %noise
                    noise = sqrt(N0/2)*(randn(1,symbol_frame_length)+1i*randn(1,symbol_frame_length));

                    % channel  ricean fading
                    riceA = sqrt(Kfactor*2*((1/sqrt(2))^2));
                    channel = (riceA +sqrt(1/2)*(randn(1,symbol_frame_length)+1i*randn(1,symbol_frame_length)))/sqrt(riceA^2+1);

                    % Simulate transmission
                    y = channel.*Modem_symbols + noise;

                    % Soft demapper
                    for j = 1:symbol_frame_length
                        for m = 1:Mlevel
                            dist_sqr = abs(y(j) - channel(j).*modulation_symbols(m))^2;
                            Symbol_prob(j,m) = -dist_sqr/N0;
                        end
                    end

                    %Compute bit probabilities
                    Bit_prob = symbol_to_bit_prob_convertor_siso(symbol_frame_length, bps, Symbol_prob);
                
                    %  Compute LLRs
                    llr =  Bit_prob(:,2) - Bit_prob(:,1);
                    llr = -llr;
                   
                    % Perform polar decoding
                    %a_hat = feval([code, '_decoder'],llr(1:length(a)),A(A_index),L,min_sum);
                    a_hat = feval([code, '_decoder'],llr,A(A_index),L,min_sum);
                    ...
                    if (mod(block_counts(end),10)==0)
                        fprintf(fid,'%f\t%e\t%i\t%i\n',EsN0,BLER,block_counts(end),block_error_counts(end));
                    end
