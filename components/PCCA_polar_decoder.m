function a_hat = PCCA_polar_decoder(e_tilde, crc_polynomial_pattern, info_bit_pattern, PC_bit_pattern, PC_circular_buffer_length, rate_matching_pattern, mode, L, min_sum, P2)
...
K = sum(info_bit_pattern & ~PC_bit_pattern);
...
bits = zeros(N, log2(N)+1); % Initialse all bits to zero. The left-most column corresponds to the decoded information, CRC, PC and frozen bits
...
for i = 1:N
    ...
    elseif PC_bit_pattern(i) == 1 % PC bit
        % The bit values come from the PC bit generators
        bits(i,1,:) = y(1,1,:);
...
for list_index = 1:min(L,2^P2)
    ...
    % kernal.
    b_hat = u_hat(info_bit_pattern & ~PC_bit_pattern);
